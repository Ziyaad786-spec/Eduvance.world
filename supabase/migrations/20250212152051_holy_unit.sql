...
-- Create policies for report card subjects
DO $$ BEGIN
...
-- Create policies for comments library
DO $$ BEGIN
...
BEGIN
  ...
  SELECT COALESCE(
    (SELECT MAX(CAST(SUBSTRING(s.student_number FROM '\d{4}-(\d+)') AS integer))
     FROM students s
     WHERE s.student_number LIKE v_year || '-%'
    ), 0) + 1
  INTO v_next_number;
  
  v_student_number := v_year || '-' || LPAD(v_next_number::text, 4, '0');
...
DROP TRIGGER IF EXISTS set_student_number_trigger ON students;
